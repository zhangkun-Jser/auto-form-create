...
class TableFieldset extends React.Component {
    ...
    getValue = () => {
        ...
        return range(rowNum).reduce((acc, index) => {
            ...
            return isHeader ? { ...acc, header: value } : { ...acc, rows: [...(acc.rows || []), { cols: value }] };
        }, {});
    };
    ...
    render() {
        ...
        const { onChange, submit, labelWidth, panelTitle, colNum: propColNum, headerFields, rowFields } = this.props;
        const {
        ...
        const ruleIndex = Array.isArray(propColNum) ? propColNum.indexOf(colNum) : 0;
        ...
        const rowTabs = indexList.map(index => {
            ...
            const isHeader = index === 0;
            const value = isHeader ? headerValue : (rowValues[rowIndex] || {}).cols;
                            ...
                            const updater = state => {
                                ...
                                return {
                                    value: isHeader
                                        ...
                                        : {
                                            ...state.value,
                                            rows: rowIndexList.map(currentRowIndex => {
